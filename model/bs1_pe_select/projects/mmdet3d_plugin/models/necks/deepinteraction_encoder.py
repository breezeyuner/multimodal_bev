...
class DeepInteractionEncoderLayer(nn.Module):
    ...
    def forward(self, img_feat, lidar_feat, img_metas, pts_metas):
        ...
        I2P_feat,I2P_P_bev_pos = self.I2P_block(lidar_feat, img_feat.view(batch_size, -1, I_C, I_H, I_W), img_metas, pts_metas)
        P2P_feat = self.P_IML(lidar_feat, lidar_feat)
        #P_Aug_feat = self.P_out_proj(torch.cat((I2P_feat, P2P_feat),dim=1))
        P_Aug_feat = self.P_out_proj(torch.cat((P2P_feat, P2P_feat),dim=1))
        new_lidar_feat = self.P_integration(torch.cat((P_Aug_feat, lidar_feat),dim=1))

        P2I_feat,P2I_I_bev_pos = self.P2I_block(lidar_feat, img_feat.view(batch_size, -1, I_C, I_H, I_W), img_metas, pts_metas)
        I2I_feat = self.I_IML(img_feat, img_feat)
        #I_Aug_feat = self.I_out_proj(torch.cat((P2I_feat.view(BN, -1, I_H, I_W), I2I_feat),dim=1))
        I_Aug_feat = self.I_out_proj(torch.cat((I2I_feat, I2I_feat),dim=1))
        new_img_feat = self.I_integration(torch.cat((I_Aug_feat, img_feat),dim=1))
        return new_img_feat, new_lidar_feat, P2I_I_bev_pos, I2P_P_bev_pos
...
class DeepInteractionEncoder(nn.Module):
    ...
    def forward(self, img_feats, pts_feats, img_metas, pts_metas):
        ...
        for i in range(self.num_layers):
            new_img_feat, new_pts_feat,P2I_I_bev_pos, I2P_P_bev_pos = self.fusion_blocks[i](new_img_feat, new_pts_feat, img_metas, pts_metas)     
        return new_img_feat, [pts_feat_conv, new_pts_feat], P2I_I_bev_pos, I2P_P_bev_pos