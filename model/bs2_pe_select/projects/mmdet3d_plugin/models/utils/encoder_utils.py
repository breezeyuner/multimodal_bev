...
class BEVWarp(nn.Module):
    ...
    def forward(self, lidar_feats, img_feats, img_metas, pts_metas, **kwargs):
        ...
        for b in range(batch_size):
            ...
            xy_bev = (xy_bev - 0.5) * 2
            #xy_bev.shape, torch.Size([6, 112, 200, 2])
            ...
            #decorated_img_feat.shape, torch.Size([6, 112, 200, 128])
            decorated_img_feat[~lift_mask]=0
            decorated_img_feats.append(decorated_img_feat.permute(0,3,1,2))
            #decorated_img_feat.permute(0,3,1,2).shape, torch.Size([6, 128, 112, 200])
            
            xyz_bev = (xyz[...,0:3] - pc_range[0:3]) / (pc_range[3:6] - pc_range[0:3]) # 0~1
            xyz_bev = (xyz_bev - 0.5) * 2 # -1 ~ 1
            xyz_bev[~lift_mask]= -2 #mask 
            xyz_bevs.append(xyz_bev)
            
        decorated_img_feats = torch.stack(decorated_img_feats, dim=0) #batch_size , [6, 128, 112, 200]
        xyz_bevs = torch.stack(xyz_bevs, dim=0)  #batch_size , [6, 112, 200, 3]
        return decorated_img_feats, xyz_bevs
...
class MMRI_P2I(nn.Module):
    ...
    def forward(self, lidar_feats, img_feats, img_metas, pts_metas, **kwargs):
        warped_img_feats,xyz_bevs = self.Warp(lidar_feats, img_feats, img_metas, pts_metas) #warped_img_feats: B, N, C, H, W
        ...
        return decorated_img_feats, xyz_bevs # xyz_bevs : B, N, H, W, 3 , corresponding position in BEV
...
class MMRI_I2P(nn.Module):
    ...
    def forward(self, lidar_feat, img_feat, img_metas, pts_metas, **kwargs):
        ...
        #lidar_feat torch.Size([1, 128, 180, 180])
        
        #print('-------------------------decorated_lidar_feat',decorated_lidar_feat.shape)
        #decorated_lidar_feat torch.Size([1, 128, 180, 180])
        xy_bevs = torch.zeros(decorated_lidar_feat.shape[0],\
                              decorated_lidar_feat.shape[2],decorated_lidar_feat.shape[3],\
                                2).to(decorated_lidar_feat.device) #B, H, W , 2
        xy_bevs[:,:,:,0] = torch.linspace(-1,1,xy_bevs.shape[1]).unsqueeze(0).unsqueeze(-1).\
                                    repeat(1,1,decorated_lidar_feat.shape[-1])
        xy_bevs[:,:,:,1] = torch.linspace(-1,1,xy_bevs.shape[2]).unsqueeze(0).unsqueeze(0).\
                                    repeat(1,decorated_lidar_feat.shape[-2],1) 
        #print('-------------------------xy_bevs',xy_bevs.shape)
        #xy_bevs torch.Size([1, 180, 180, 2])
        
        return decorated_lidar_feat,xy_bevs
