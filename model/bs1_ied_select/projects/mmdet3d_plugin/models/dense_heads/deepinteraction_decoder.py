...
class DeepInteractionDecoder(nn.Module):
    ...
    def forward(self, pts_inputs, img_inputs, img_metas, P_bev_pos, I_bev_pos,):
        ...
        P_bev_pos_feat_flatten = self.extr_lidar_posembed(self.pos2embed(P_bev_pos_flatten)).permute(0, 2,1 )
        ...
        I_bev_pos_flatten = I_bev_pos.view(batch_size, self.num_views, -1, 2)
        ...
        I_bev_pos_feat_flatten = self.extr_image_posembed(self.pos2embed(I_bev_pos_flatten)).permute(0, 1, 3, 2 )
        ...
        img_feat_flatten_PE = img_feat_flatten 
        ...
        new_lidar_feat_PE = new_lidar_feat 
        ...
        for layer_idx in range(self.num_mmpi):
            ...
            if layer_idx % 2 == 0: # 0: Image, 1: lidar 
                query_pos_feat = self.extr_image_posembed(self.pos2embed(query_pos)).permute(0, 2, 1 )
            else :
                query_pos_feat = self.extr_lidar_posembed(self.pos2embed(query_pos)).permute(0, 2, 1 )
