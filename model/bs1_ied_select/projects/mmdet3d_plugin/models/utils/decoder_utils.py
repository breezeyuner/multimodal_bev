...
class ImageRCNNBlock(nn.Module):
    ...
    def forward(self, query_feat, res_layer, 
                org_new_lidar_feat, org_img_feat_flatten,
                new_lidar_feat, img_feat_flatten,
                img_metas, img_h, img_w, **kwargs):
        ...
        org_roi_features = torch.zeros(query_feat.shape[0], query_feat.shape[1], query_feat.shape[2], 7,7 ).to(query_feat.device) #B, C, prop_num , output_size=7
        ...
        for sample_idx in range(batch_size):
            ...
            for view_idx in range(self.num_views):
                ...
                #print('---------------------query_feat_view',query_feat_view.shape)
                ...
                #ROI feat without PE  
                org_roi_features_view = self.pooler([org_img_feat_flatten[sample_idx:sample_idx + 1,
                                                                        view_idx].reshape(1, -1, img_h, img_w)],
                                            [Boxes(circumscribed_rectangle_on_feature_coor)])
                #print('---------------------org_roi_features_view',org_roi_features_view.shape)
                #[12, 128, 7,7]
                org_roi_features_view = org_roi_features_view.permute(1,0,2,3)
                org_roi_features[sample_idx, :, on_the_image,:,:] = org_roi_features_view.clone() #[B, 128, 200,  7, 7]
                
        return query_feat, on_the_image_mask, org_roi_features
...
class PointRCNNBlock(nn.Module):
    ...
    def forward(self, query_feat, res_layer, 
        ...
        for sample_idx in range(batch_size):
            ...
            #ROI feat without PE  
            org_roi_features_view = self.pooler_pts([org_new_lidar_feat[sample_idx:sample_idx + 1]],
                                   [Boxes(circumscribed_rectangle_on_feature_coor[sample_idx])])
                                   
            org_roi_features_view = org_roi_features_view.permute(1,0,2,3)  #[128, 200, 7,7 ] 

            org_roi_features[sample_idx, :, : ,:,:] = org_roi_features_view.clone() #[B,128, 200, 7,7 ] 
            
        return query_feat, None, org_roi_features
