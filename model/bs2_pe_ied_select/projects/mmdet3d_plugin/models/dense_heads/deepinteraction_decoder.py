...
class DeepInteractionDecoder(nn.Module):
    def __init__(self,
        ...
        for i in range(int(self.num_mmpi/2)):
            ...
            self.pred_head.append(FFN(hidden_channel * 3, heads, conv_cfg=conv_cfg, norm_cfg=norm_cfg, bias=bias))
        ...
        #-------------------------------------------------------------------
        #extra embedding for image/LiDAR feat 
        
        self.hidden_channel = hidden_channel
        self.extr_image_posembed = nn.Sequential(
                    nn.Linear(hidden_channel * 3, hidden_channel * 4),
                    nn.ReLU(),
                    nn.Dropout(0.2),
                    nn.Linear(hidden_channel * 4, hidden_channel * 2),
                    nn.ReLU(),
                    nn.Dropout(0.2),
                    nn.Linear(hidden_channel * 2, hidden_channel * 1),
                )
        
        self.extr_lidar_posembed = nn.Sequential(
        ...
        #input #[B, 128, 200,  7, 7] -> ([B*200, 128, 7,7 ]) ->
        #output #[B* 200, 128] -> [B, 200, 128]
        
        #self.image_roi_feat_res = nn.Sequential(
        #            nn.Conv2d(hidden_channel, hidden_channel , (3,3) ), #[B* 200, 128, 5, 5 ]
        #            nn.Conv2d(hidden_channel, hidden_channel , (3,3) ), #[B* 200, 128, 3, 3 ]
        #            nn.Conv2d(hidden_channel, hidden_channel , (3,3) ), #[B* 200, 128, 1, 1 ]
        #            nn.Flatten(1,3), #[B*200,  128 ]
        #            nn.Linear(hidden_channel , hidden_channel * 2),
        #            nn.LayerNorm(hidden_channel * 2),
        #            nn.ReLU(inplace=True),
        #            nn.Linear(hidden_channel * 2, hidden_channel) #[B* 200, 128]
        #        )
        #
        #self.lidar_roi_feat_res = nn.Sequential(
        #            nn.Conv2d(hidden_channel, hidden_channel , (3,3) ), 
        #            nn.Conv2d(hidden_channel, hidden_channel , (3,3) ), 
        #            nn.Conv2d(hidden_channel, hidden_channel , (3,3) ), 
        #            nn.Flatten(1,3) ,
        #            nn.Linear(hidden_channel , hidden_channel * 2),
        ...
        ##simple residual
        #self.image_roi_feat_res = nn.Sequential(
        #    nn.Conv2d(hidden_channel, hidden_channel , (7,7) ), #[B* 200, 128, 1, 1 ]
        #    nn.Flatten(1,3) #[B*200,  128 ]
        #    )
        #
        #self.lidar_roi_feat_res = nn.Sequential(
    ...
    #CMT
    def image_posembed(self, in_ ):
        # in_ torch.Size([1, 6, 22400, 3])
        out_ = self.extr_image_posembed(self.pos2embed3d(in_)) #torch.Size([1, 6, 22400, 128])
        
        mask = torch.ones_like(in_)
        mask_zero = torch.zeros_like(in_) 
        mask = torch.where((in_ < -1) | (in_ >  1), mask_zero , mask)
        
        mask = mask.sum(-1, keepdim=True) # [1, 6, 22400, 1]
        mask_zero = torch.zeros_like(mask) 
        mask_one = torch.ones_like(mask) 
        
        mask = torch.where(mask != 3 , mask_zero , mask_one)
        out_ = out_ * mask
        
        return out_
        
    def lidar_posembed(self, in_ ):
        # in_ torch.Size([1, 32400, 2])
        out_ = self.extr_lidar_posembed(self.pos2embed(in_)) #torch.Size([1, 32400, 128])
        ...
        mask = torch.where(mask != 2 , mask_zero , mask_one)
        out_ = out_ * mask
        
        return out_
        
    def pos2embed(self,pos, temperature=10000):
        num_pos_feats=self.hidden_channel
    ...
    def pos2embed3d(self,pos, temperature=10000):
        num_pos_feats=self.hidden_channel
        assert num_pos_feats%2 ==0
    ...
    def forward(self, pts_inputs, img_inputs, img_metas, P_bev_pos, I_bev_pos,):
        ...
        P_bev_pos_flatten = P_bev_pos.view(batch_size,-1, 2)  # [BS, H*W, 2]
        #print('---------------------lidar_feat_flatten',lidar_feat_flatten.shape )
        #lidar_feat_flatten torch.Size([1, 128, 32400])
        #print('---------------------P_bev_pos_flatten',P_bev_pos_flatten.shape )
        #P_bev_pos_flatten torch.Size([1, 32400, 2])
        P_bev_pos_feat_flatten = self.lidar_posembed( P_bev_pos_flatten).permute(0, 2,1 )
        #print('---------------------P_bev_pos_feat_flatten',P_bev_pos_feat_flatten.shape )
        #P_bev_pos_feat_flatten torch.Size([1, 128, 32400])
        ...
        #query_feat torch.Size([1, 128, 200])
        ...
        #query_pos torch.Size([1, 200, 2])
        
        #lidar feature + lidar pe 
        lidar_feat_flatten = lidar_feat_flatten + P_bev_pos_feat_flatten


        #query from lidar 
        #query_pos_feat = self.extr_lidar_posembed(self.pos2embed(query_pos)).permute(0, 2, 1 )
        query_pos_feat = self.lidar_posembed( query_pos ).permute(0, 2, 1 )
        query_feat = query_feat + query_pos_feat
        ...
        I_bev_pos_flatten = I_bev_pos.view(batch_size, self.num_views, -1, 3)
        
        #print('---------------------img_feat_flatten',img_feat_flatten.shape )
        #img_feat_flatten torch.Size([1, 6, 128, 22400])
        #print('---------------------I_bev_pos_flatten',I_bev_pos_flatten.shape )
        #I_bev_pos_flatten torch.Size([1, 6, 22400, 2])
        
        #I_bev_pos_feat_flatten = self.extr_image_posembed(self.pos2embed3d(I_bev_pos_flatten)).permute(0, 1, 3, 2 )
        I_bev_pos_feat_flatten = self.image_posembed( I_bev_pos_flatten ).permute(0, 1, 3, 2 )
        #print('---------------------I_bev_pos_feat_flatten',I_bev_pos_feat_flatten.shape )
        #I_bev_pos_feat_flatten torch.Size([1, 6, 128, 22400])
        #image feat + image PE 
        img_feat_flatten_PE = img_feat_flatten + I_bev_pos_feat_flatten
        
        
        if self.img_feat_pos is None:
            (h, w) = img_inputs.shape[-2], img_inputs.shape[-1]
            img_feat_pos = self.img_feat_pos = self.create_2D_grid(h, w).to(img_feat_flatten_PE.device)
        else:
            img_feat_pos = self.img_feat_pos
        
        self.on_the_image_mask = []
        
        #print('---------------------new_lidar_feat',new_lidar_feat.shape )
        #new_lidar_feat torch.Size([1, 128, 180, 180])
        #P_bev_pos_feat = self.extr_lidar_posembed(self.pos2embed(P_bev_pos)).permute(0,3,1,2) 
        P_bev_pos_feat = self.lidar_posembed( P_bev_pos ).permute(0,3,1,2) 
        #new_lidar_feat + lidar PE 
        new_lidar_feat_PE = new_lidar_feat + P_bev_pos_feat

        
        query_pos = res_layer['center'].detach().clone().permute(0, 2, 1)
        query_hight = res_layer['height'].detach().clone().permute(0, 2, 1)
        pc_range = query_pos.new_tensor([-54, -54, -5, 54, 54, 3])
        
        
        query_pos_norm = query_pos.clone()
        query_pos_norm[...,0] = query_pos_norm[...,0]/ (self.grid_x_size -1 )         
        query_pos_norm[...,1] = query_pos_norm[...,1]/ (self.grid_y_size -1 ) # 0~1        
        query_pos_norm = (query_pos_norm - 0.5) * 2  # -1 ~ 1        
        
        query_pos3d_norm = torch.cat((query_pos_norm,query_hight),dim=-1).clone()
        query_pos3d_norm[...,2:3]  = (query_pos3d_norm[...,2:3] - pc_range[2:3]) / (pc_range[5:6] - pc_range[2:3]) # 0 ~ 1
        query_pos3d_norm[...,2:3] = ( query_pos3d_norm[...,2:3] - 0.5 ) * 2  # -1 ~ 1
        
        query_pos_feat = self.image_posembed(query_pos3d_norm).permute(0, 2, 1 ).clone() +\
                         self.lidar_posembed( query_pos_norm ).permute(0, 2, 1 ).clone()
        ...
        for layer_idx in range(self.num_mmpi):
            ...
            query_feat, on_the_image, org_roi_feat = self.decode_head[layer_idx](
                query_feat=prev_query_feat, #+ query_pos_feat,  #query_feat_PE  prev_query_feat + query emb 
                res_layer=res_layer, 
                org_new_lidar_feat=new_lidar_feat, org_img_feat_flatten=img_feat_flatten,
                new_lidar_feat=new_lidar_feat_PE, img_feat_flatten=img_feat_flatten_PE,
                img_metas=img_metas, img_h=img_h, img_w=img_w
            )
            
            #new_lidar_feat torch.Size([1, 128, 180, 180])
            #img_feat.view(batch_size, self.num_views, img_feat.shape[1], -1)
            #img_feat_flatten torch.Size([1, 6, 128, 22400]) 
            
            #print('---------------------query_feat',query_feat.shape) 
            #query_feat torch.Size([1, 128, 200])
            #print('---------------------prev_query_feat',prev_query_feat.shape)
            #prev_query_feat torch.Size([1, 128, 200])
            
            #org_roi_feat: ROI feature sampled from image/lidar feature without PE directly
            #print('---------------------org_roi_feat',org_roi_feat.shape)
            #[B,128, 200, 7,7 ] 
            
            #---------------------------------------------------------------
            #residual net 
            org_roi_feat = org_roi_feat.permute(0,2,1,3,4).view(-1, self.hidden_channel,7,7) #[B*200, 128, 7,7 ]
            if layer_idx % 2 == 0: # 0: Image, 1: lidar 
                org_roi_feat = self.image_roi_feat_res(org_roi_feat)
            else:
                org_roi_feat = self.lidar_roi_feat_res(org_roi_feat)
            org_roi_feat = org_roi_feat.view(-1, 200, self.hidden_channel ) #[B,200, 128 ]
            org_roi_feat = org_roi_feat.permute(0,2,1) #[B, 128, 200]
            
            res_layer = self.pred_head[layer_idx](torch.cat([query_feat, prev_query_feat, org_roi_feat], dim=1))  #pass to FFN(hidden_channel*3)
            res_layer['center'] = res_layer['center'] + query_pos.permute(0, 2, 1)
            if layer_idx % 2 == 0:
